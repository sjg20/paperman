...
#
#    scripts/fakescan.sh DIR ./paperman
...
# the tests, by 'qmake CONFIG+=test', in test/fakescan under the build
# directory; set FAKESCAN_LIBDIR if that is not the source directory.
...
if [ ! -f "$libdir/libsane-fakefujitsu.so.1" ]; then
   echo "no fake scanner in $libdir: build with 'qmake CONFIG+=test'" >&2
   exit 1
fi
mkdir -p "$dir/hopper"
