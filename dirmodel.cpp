...
Diritem::Diritem(const QString& path, bool recent, QObject *parent)
...
bool Diritem::setDir(const QString& dir)
   {
//   QModelIndex ind;
   QDir qd (dir);
...
=======
bool Dirmodel::addDir(const QString& dir, bool ignore_error)
>>>>>>> 7d447be (wip)
   {
...
QModelIndex Dirmodel::index(int row, int column, const QModelIndex &parent)
   ...
   if (item)
      ...
      ind = createIndex(row, column, item);
...
Diritem * Dirmodel::findItem(const QModelIndex &index) const
{
   QModelIndex root = findRoot(index);

   if (root.isValid() && root.row() > 0 && root.row() < _item.size())
      return _item[root.row()];
