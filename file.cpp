...
void File::updateFilename (const QString &fname)
   ...
   if (_ext == ".max")
      ...
      _basename = _filename;
   qDebug () << _dir << _filename << _basename;
   }
...
err_info *File::copyTo (File *fnew, int odd_even, Operation &op, bool verbose)
   ...
   for (pagenum = 0; pagenum < page_count; pagenum++)
      ...
      QByteArray ba = QByteArray::fromRawData ((const char *)image.bits (), image.numBytes ());
...
//    image.save ("/tmp/1.jpg");
   ...
   qDebug () << "thumbnail" << image.numBytes () << ba.size ();
   return ba;
   }
